...
import StepsContent from '@/app/registration-form/application/[[...steps]]/page';
    ...
    <MbisProvider>
      ...
      <div>
        <section className={`max-w-[2000px] m-auto ${whiteSpaces.paddingY}`}>
          <main
            className={`${whiteSpaces.paddingX} xmd:pt-[89px] md:pt-[79px]`}
          >
            {payment.map((itm) => (
              <div className="flex gap-9" key={itm.id}>
                {itm.content.map((cont, index) => (
                  <Paragraph
                    type="global"
                    key={`${cont.id}--`}
                    classname={`font-dejavu xmd:pb-10 md:pb-[58px] cursor-pointer ${
                      selectedNumber === index
                        ? 'font-bold text-black text-Bold-1-clamp'
                        : 'text-[#848484]'
                    }`}
