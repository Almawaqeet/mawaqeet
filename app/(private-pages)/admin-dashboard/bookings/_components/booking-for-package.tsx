...
import { useGetBookingFinancialSummaryForASpecificPackage, useGetBookingsForAPackage } from "@/api/services/booking";
...
import { Calendar, CreditCard, Mail, Phone, User, DollarSign, Star, Diamond, Gem, Clock } from "lucide-react";
...
import { booking_payment_status_constant, package_price_category } from "@/constants/generic";
...
export default function BookingsForSpecificPackage() {
  ...
  const { data: financialSummaryData, isLoading: summaryLoading } = useGetBookingFinancialSummaryForASpecificPackage(packageId as string);
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {summaryLoading ? (
            ...
            Object.entries(financialSummaryData.summary).map(([category, summary]) => (
              <Card key={category} className="border-gray-200 shadow-sm rounded-xl hover:shadow-md transition-shadow duration-200">
                <CardContent className="p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`p-2.5 rounded-lg ${
                      category.toLowerCase() === package_price_category.VIP ? 'bg-amber-100' :
                      category.toLowerCase() === package_price_category.DELUXE ? 'bg-emerald-100' :
                      'bg-indigo-100'
                    }`}>
                      {category.toLowerCase() === package_price_category.VIP ? (
                        <Star className="h-5 w-5 text-amber-600" />
                      ) : category.toLowerCase() === package_price_category.DELUXE ? (
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gray-50 p-3 rounded-lg">
                        ...
                        <p className="text-base font-medium text-gray-900">₦{summary?.total_amount_paid?.toLocaleString() ?? 'N/A'}</p>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-600 mb-1">Amount Due</p>
                        <p className="text-base font-medium text-gray-900">₦{summary?.total_amount_due?.toLocaleString() ?? 'N/A'}</p>
                      </div>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <p className="text-sm text-gray-700 mb-3">Booking Status</p>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="bg-white p-2 rounded-lg">
                          <p className="text-lg font-medium text-green-600">{summary?.total_bookings?.completed ?? '0'}</p>
                          <p className="text-xs text-gray-600">Completed</p>
                        </div>
                        <div className="bg-white p-2 rounded-lg">
                          <p className="text-lg font-medium text-yellow-600">{summary?.total_bookings?.installment ?? '0'}</p>
                          <p className="text-xs text-gray-600">Installment</p>
                        </div>
                        <div className="bg-white p-2 rounded-lg">
                          <p className="text-lg font-medium text-gray-600">{summary?.total_bookings?.no_payment ?? '0'}</p>
            ...
            <div className="col-span-3 text-center py-8 text-gray-500">No financial summary available.</div>
          )}
        </div>
      </div>
