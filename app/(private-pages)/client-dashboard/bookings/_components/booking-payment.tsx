...
export function BookingPayment({ id }: BookingPaymentProps) {
  ...
  const { mutate: initiatePayment, isPending: isInitiatingPayment } =
    useInitiateBookingPayment(id);
  const { data: verifyPaymentData, isPending: isVerifyingPayment } =
    useVerifyBookingPayment(reference);
  const {
    mutate: makeBookingThroughWallet,
    data: payingThroughWalletData,
    isPending: isPayingThroughWallet,
  } = useMakeBookingPaymentThroughWallet(id);
  ...
  const handlePayment = () => {
    ...
    if (isFullPayment && amount < remainingAmount) {
      showToast({
        title: 'Error',
        description:
          'Full payment plan requires complete payment of remaining balance',
        variant: 'destructive',
      });
      return;
    }
    ...
    >
      ...
      <AppModal
        open={
          ...
          reference === 'wallet-payment-success'
        }
      ...
      >
        <div className="flex flex-col items-center justify-center gap-4">
          ...
          payingThroughWalletData?.receipt_url ? (
            <a
              href={
                verifyPaymentData?.receipt_url ??
                payingThroughWalletData?.receipt_url
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center py-3 border-b hover:bg-gray-50 px-2 rounded transition-colors">
                  ...
                  <span className="font-medium">
                    {booking?.payment_plan?.toUpperCase() ?? ''}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="mt-6 space-y-4">
                  ...
                  >
                    ...
                    {isFullPayment && (
                      <span className="text-red-500 text-sm">
                        (Full payment of ₦{remainingAmount?.toLocaleString()}{' '}
                        required)
                      </span>
                    )}
                  </Label>
                ...
                >
                  <AppButton
                    ...
                    disabled={
                      isInitiatingPayment ||
                      isPayingThroughWallet ||
                      !paymentAmount
                    }
                    loading={isInitiatingPayment || isPayingThroughWallet}
                  >
                    {isInitiatingPayment || isPayingThroughWallet
                      ? 'Processing...'
                      : 'Pay Now'}
                  </AppButton>
                </motion.div>
