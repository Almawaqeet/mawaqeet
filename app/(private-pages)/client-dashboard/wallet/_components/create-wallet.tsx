...
export default function CreateWallet() {
  ...
  const {
    mutate: verifyWalletAccountNumber,
    isPending: isVerifyingWalletAccountNumber,
  } = useVerifyWalletAccountNumber();
  ...
  const formik = useFormik({
    ...
    validationSchema,
    onSubmit: (values) => {
      const selectedBank = bankList?.banks?.find(
        (bank) => bank.bank_code === values.bank
      );
      if (!selectedBank) {
  ...
  const handleConfirm = () => {
    if (accountName) {
      const selectedBank = bankList?.banks?.find(
        (bank) => bank.bank_code === formik.values.bank
      );
      if (selectedBank) {
        createWallet(
          {
              ...
              if (response) {
                if (typeof window !== undefined) {
                  localStorage.setItem(
                    LOCAL_STORAGE_KEYS.USER_WALLET_STATUS,
                    'found'
                  );
                }
                showToast({
        ...
        <CardContent>
          <form onSubmit={formik.handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="account_number" className="font-medium">
                Account Number
              </Label>
              <Input
                id="account_number"
                ...
                >
                  ...
                  <SelectContent>
                    {bankList?.banks?.map((bank) => (
                      <SelectItem
                        key={bank?.bank_code}
                        value={bank?.bank_code as string}
                      >
                        {bank?.name}
                      </SelectItem>
                    ))}
            ...
            >
              {isCreatingWallet || isVerifyingWalletAccountNumber
                ? 'Creating...'
                : 'Create Wallet'}
            </AppButton>
          </form>
