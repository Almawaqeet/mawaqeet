...
export default function VipPackageModal() {
  const {
    state: { openVipPackage },
    dispatch,
  } = useMbisContext();

  const handleClose = () => {
    dispatch({ type: 'openVipPackage', payload: false });
  };

  const vip_package = packages.find((itm) => itm.id === 'vip package')
  return (
    <div>
      <Modal
        open={openVipPackage as boolean}
