...
export default function DelauxePackageModal() {
  const {
    state: { openDelauxePackage },
    dispatch,
  } = useMbisContext();

  const handleClose = () => {
    dispatch({ type: 'openDelauxePackage', payload: false });
  };

  const delauxe_package = packages.find((itm) => itm.id === 'deluxe package')
  return (
    <div>
      <Modal
        open={openDelauxePackage as boolean}
