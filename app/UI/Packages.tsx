...
const Packages: React.FC<PackagesProps> = ({
  ...
  return (
    <div
      className={`bg-[#F8F8F8] drop-shadow-white-ash xmd:rounded-ee-xl xmd:rounded-ss-xl package flex flex-col xmd:gap-4 relative xmd:py-8 xmd:px-4 group hover:bg-hover-color transition-all duration-[0.5s] ease-[cubic-bezier(0.645,0.045,0.355,1)] delay-[400] mobile:h-[900px] sm:h-[850px] md:h-[850px] cursor-pointer lg:min-h-[750px] ${offmainheight} ${
        index === 1
          ? 'xmd:pb-[6.8rem] '
          : index === 2
          ? 'xmd:pb-[11rem] '
    ...
    >
      <div
        className={`${morestyle} rounded-lg border-2 border-[#D9D9D9] p-4 m-auto bg-white w-full`}
      >
        <div className="flex justify-between">
          <Formik
            ...
            onSubmit={(
              values,
              { setSubmitting }: FormikHelpers<RadioInitialValues>
            ) => {
              if (!values) return;
              handleNext();
              console.log(values);
          ...
          <h1 className="xmd:pb-11 text-end font-dejavu font-bold">
            {package_name}
          </h1>
        </div>

        <div className={`${prices}`}>
          <div className="flex flex-col xmd:gap-6 md:gap-8 text-end font-dejavu justify-center items-center">
      ...
      <h3
        className={`text-fz-md text-center group-hover:text-accordion font-dejavu pb-14 ${offcontent}`}
      >
        {title_head}
      </h3>
      <div className="grid xmd:gap-4 mobile:gap-5">
        <p
          className={`text-team-clamp group-hover:text-accordion font-dejavu ${offcontent}`}
        >
          {title_intro}
        </p>
        <ul className="flex flex-col gap-6">
          {bullets.map((bullet, idx) => (
            <li
              key={idx}
              className="flex gap-2 group-hover:text-accordion font-dejavu"
            >
