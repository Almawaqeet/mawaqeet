...
const FormInput = (props:FormInputProps) => {
  return (
    <div className={`flex flex-col xmd:gap-3 xmd:pb-6 ${props.className}`}>
      ...
      <input name={props.name} onClick={props.onClick} type={props.type} placeholder={props.placeholder} className={`w-full border-[0.5px] border-[#A0A8AD] xmd:rounded-lg md:rounded-md lg:rounded-lg xmd:h-12 lg:h-[60px] md:h-8 xmd:placeholder:text-fz-xsm md:placeholder:text-fz-xss placeholder:font-dejavu placeholder:text-[#D9D9D9] placeholder:pl-4 ${props.inputStyle}`} />
