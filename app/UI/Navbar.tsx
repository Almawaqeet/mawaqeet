...
const MobileNavMenu: React.FC<MobileNavMenuProps> = ({ isActive, navItems, activeIndex, dispatch, dropdownmobile, arrow, dropdownItems, subNavActiveIndex, navbtn }) => (
  isActive && (
    <div
      className={` fixed z-10 min-h-svh h-screen max-h-[1000px] overflow-y-scroll bg-${brandColors.white} drop-shadow-white-ash lg:hidden tab_md:hidden right-0 top-16 xmd:left-[20%] translate-x-0 ${
        ...
        <ul className="flex justify-between items-center font-dejavu lg:gap-12 md:gap-10 text-navbar-clamp font-normal md:leading-5">
          ...
          <div className="flex items-center">
            <Link href={'/login'}>
              <Button
                onClick={() => dispatch({ type: 'togglenavbtn' })}
                className="mobile:hidden xmd:hidden tab_md:block"
                variant={navbtn ? "default" : "outline"}
                icon={<IoMdArrowRoundForward />}
                isLoading={true}
              >
