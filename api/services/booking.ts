...
import { BookingFinancialSummaryResponse, BookingInformationResponse, BookingVerifyPaymentResponse, CustomApiResponse, InitiateBookingApiResponse, InitiateBookingPaymentRequest, InitiateBookingRequest, InitiatePaymentResponse, PackageBookingListResponse, PaginatedResponse, SimpleBookingResponse } from "@/api/types";
...
export const useGetBookingsForAPackage = (packageId: string, params?: { search?: string, page?: number, status?: string, category?: string }) => {
    const baseQueryKey = generateBaseQueryKeyFromRoute(routes.bookings.bookingsForPackage(packageId));
    return useAppQueryWithPaginationAndParams<PaginatedResponse<PackageBookingListResponse>>({
        apiRoute: routes.bookings.bookingsForPackage(packageId),
        queryKey: [baseQueryKey, params?.search, params?.page, params?.status, params?.category],
        params: params
    });
}


export const useGetBookingFinancialSummaryForASpecificPackage = (packageId: string) => {
    const baseQueryKey = generateBaseQueryKeyFromRoute(routes.bookings.bookingFinancialSummaryForASpecificPackage(packageId));
    return useAppQuery<BookingFinancialSummaryResponse>({
        apiRoute: routes.bookings.bookingFinancialSummaryForASpecificPackage(packageId),
        queryKey: [baseQueryKey]
    });
}
