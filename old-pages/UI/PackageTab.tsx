...
const PackageTab = () => {
    const { btn1Ref, btn2Ref, btn3Ref, btn4Ref, activeBtnServiceId, handleButtonClick } = useTab();
    ...
    return (
        <>
            {selectedComponent && <Tabs defaultValue="account" className="w-[400px]">
                <TabsList>
                    <TabsTrigger value="account">
                        {isTabDisplay === btn1Ref ? 'VIP' :
                            isTabDisplay === btn2Ref ? 'Delauxe' :
                                isTabDisplay === btn3Ref ? 'Standard' :
                                    isTabDisplay === btn4Ref ? 'Saving Scheme' :
                                        'VIP'}
                    </TabsTrigger>
