...
const Step1: React.FC = () => {
  ...
  const [existingEmail, setExistingEmail] = React.useState<string | null>(
    'user@example.com'
  ); // Replace with actual existing email
  ...
  return (
    <React.Fragment>
      <Headings
        type="global"
        classname="font-bold xmd:leading-[18px] tracking-tight xmd:pb-3 md:pb-5 lg:pb-8 text-center  "
      >
        Application Form
      </Headings>
      ...
      <Paragraph
        type="global"
        classname="  xmd:text-fz-xss md:text-fz-sm lg:text-fz-md text-center text-[#848484] xmd:pb-[55px] md:pb-[66px]"
      >
        Provide all the required Information
      </Paragraph>

      <div
        className={` xmd:px-5 px-[20px] sm:px-[16px] md:px-12 lg:px-[150px] xl:px-[150px] 2xl:px-[150px] xmd:pb-[37px] lg:pb-[126px]`}
      >
        <div className="bg-[#F8F8F8] rounded-lg xmd:w-full xmd:p-7 flex xmd:flex-col md:grid md:grid-cols-[200px_minmax(_4px,_8px)_1fr] lg:grid-cols-[300px_minmax(_6px,_12px)_1fr] justify-cente md:gap-3">
          <div>
            <Paragraph
              type="globalBold"
              classname="xmd:pb-7 md:pb-8 font-bold text-quote-clamp  "
            >
              Guideline:-
            </Paragraph>
            ...
            {steps.map((itm) =>
              itm.content.map((item) => (
                <Paragraph
                  type="globalBold"
                  key={`${item.id}--item`}
                  classname="text-team-clamp xmd:font-normal leading-4 tracking-tight   xmd:pb-6 md:pb-8"
                >
                  {item.item}
                </Paragraph>
              ))
          ...
          <div className="bg-white border-2 border-[#333333] border-opacity-20 rounded-lg grid xmd:grid-cols-1 xmd:p-4 xmd:gap-4 md:grid-cols-[1fr_minmax(_5px,_8px)_1fr]">
            <div className="md:flex md:flex-col md:gap-8">
              <div className="flex justify-between md:flex-col xmd:pb-4 md:pb-0 md:justify-center md:gap-8">
                <Paragraph
                  type="globalBold"
                  classname="text-team-clamp font-bold leading-5 tracking-tight  "
                >
                  Watch Full Video
                </Paragraph>
                ...
                <div className="bg-accordion flex xmd:gap-[7px] xmd:w-2/6 md:w-2/4 px-2 xmd:h-3/4 justify-between items-center">
                  ...
                  <Paragraph
                    type="globalBold"
                    classname="xmd:text-fz-xxs lg:text-fz-xss md:text-fz-xsm text-inherit  "
                  >
                    1 Min 14 Secs
                  </Paragraph>
                </div>
        ...
        >
          {({ handleSubmit, values, errors, touched }) => (
            <Form onSubmit={handleSubmit}>
              <div
                className={` xmd:px-5 px-[20px] sm:px-[16px] md:px-12 lg:px-[150px] xl:px-[150px] 2xl:px-[150px] grid xmd:grid-cols-1 md:grid-cols-2 md:gap-x-20`}
              >
              ...
              <BtnGlobal
                type="submit"
                className="  bg-[#4B3938] text-white xmd:px-[27px] xmd:py-[10px] rounded-lg m-auto"
              >
